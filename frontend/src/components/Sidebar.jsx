...
const Sidebar = () => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl text-white ${user ? "max-md:hidden":""}`}>
        <div className='pb-5'>
        ...
        <div className='flex flex-col'>
            {filteredUsers.map((user,index)=>(
                <div onClick={()=>{setselecteduser(user);setunseenmessages((prev)=>{const updated={...prev};delete updated[user._id];return updated})}} key={user._id} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selecteduser?._id===user._id ? 'bg-[#282142]/50':''}`}>
